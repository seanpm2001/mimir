std.manifestYamlDoc({
  ...
  write:: {
    ...
    'mimir-write-zone-c-61': mimirService({
      ...
      extraArguments: [
        ...
        '-ingest-storage.kafka.replay-shards=384',
        '-ingest-storage.kafka.batch-size=256',
        '-ingest-storage.kafka.replay-concurrency=4',
        '-ingest-storage.kafka.records-per-fetch=12288',
      ],
  ...
  local mimirService(serviceOptions) = {
    ...
    command: [
      ...
      '-c',
      std.join(' ', [
        (if defaultOptions.debug then 'exec ./dlv exec ./mimir --listen=:%(debugPort)d --headless=true --api-version=2 --accept-multiclient --continue -- ' % options else 'exec ./mimir'),
